...
{
    /// <summary>
    /// Port of Warband's rglPerlin.cpp.
    /// Implements 3D Perlin noise with specific permutation/gradient tables.
    /// </summary>
    public static class RglPerlin
    {
        private static readonly byte[] PermutationTable = new byte[]
        {
        ...
        private static int CLAMP(int x) => (byte)(x & 255);
        private static int PERMUTATION(int x) => PermutationTable[CLAMP(x)];
        
        // Gradient table is flattened (256 * 3 floats). 
        // Index is 3 * PERMUTATION(x)
        private static Vector3 GRADIENT(int x)
        {
            int idx = 3 * PERMUTATION(x);
            return new Vector3(GradientTable[idx], GradientTable[idx + 1], GradientTable[idx + 2]);
        }

        public static Vector4 Octave(float frequency, float gain, int numOctaves, bool adjust, Vector4 input)
        {
            Vector4 noiseSum = Vector4.zero;
            float amplitude = 1.0f;
            float norm = 0.0f;

            for (int i = 0; i <= numOctaves; ++i)
            {
                // Note: input is a Vector4, allowing 4D input or just 3D with w=0
                Vector4 rawNoise = Compute(amplitude * frequency, input);

                if (adjust)
                {
                    float sum = rawNoise.x + rawNoise.y + rawNoise.z;
                    if (sum >= 0.0f)
                        rawNoise *= (sum + 2.0f);
                    else
                        rawNoise *= (sum - 2.0f);
                ...
                noiseSum += rawNoise * amplitude;
                norm += amplitude;
                amplitude *= gain;
                gain = 0.61f; // Specific Warband constant
            }

            return noiseSum / norm;
        }

        public static Vector4 Compute(float size, Vector4 input)
        {
            ...
            float xd = (input.x + 1.0f) / size;
            float yd = (input.y + 1.0f) / size;
            float zd = (input.z + 1.0f) / size;

            int xdi = Mathf.FloorToInt(xd);
            int ydi = Mathf.FloorToInt(yd);
            int zdi = Mathf.FloorToInt(zd);
            ...
            int xp = PERMUTATION(xdi);
            int xp1 = PERMUTATION(xdi + 1);
            int xpy = PERMUTATION(xp + ydi);
            int xpy1 = PERMUTATION(xp + ydi + 1);
            int xp1y = PERMUTATION(xp1 + ydi);
            int xp1y1 = PERMUTATION(xp1 + ydi + 1);

            int zp = CLAMP(zdi);     // Note: Warband code calls RGL_PERLIN_CLAMP(zdi) here
            int zp1 = CLAMP(zdi + 1);

            float xmm3m = 1.0f - 3.0f * xmm + (xmm * 2) * xm; // 1 - 3x^2 + 2x^3  (Smoothstep-like)
            ...
            // These are blend factors
            float pxyz = xmm3m * ymm3m * zmm3m;
            float pxyz1 = xmm3m * ymm3m * zm1m3m; // z+1
            float pxy1z = xmm3m * ym1m3m * zmm3m; // y+1
            float pxy1z1 = xmm3m * ym1m3m * zm1m3m; // y+1, z+1

            float px1yz = xm1m3m * ymm3m * zmm3m; // x+1
            float px1yz1 = xm1m3m * ymm3m * zm1m3m; // x+1, z+1
            float px1y1z = xm1m3m * ym1m3m * zmm3m; // x+1, y+1
            float px1y1z1 = xm1m3m * ym1m3m * zm1m3m; // x+1, y+1, z+1

            Vector3 p3d;

            p3d = GRADIENT(xpy + zp);
            outVec.x += xm * pxyz * p3d.x;
            outVec.y += ym * pxyz * p3d.y;
            outVec.z += zm * pxyz * p3d.z;

            p3d = GRADIENT(xpy + zp1);
            outVec.x += xm * pxyz1 * p3d.x;
            outVec.y += ym * pxyz1 * p3d.y;
            outVec.z += zm1 * pxyz1 * p3d.z;

            p3d = GRADIENT(xpy1 + zp);
            outVec.x += xm * pxy1z * p3d.x;
            outVec.y += ym1 * pxy1z * p3d.y;
            outVec.z += zm * pxy1z * p3d.z;

            p3d = GRADIENT(xpy1 + zp1);
            outVec.x += xm * pxy1z1 * p3d.x;
            outVec.y += ym1 * pxy1z1 * p3d.y;
            outVec.z += zm1 * pxy1z1 * p3d.z;

            p3d = GRADIENT(xp1y + zp);
            outVec.x += xm1 * px1yz * p3d.x;
            outVec.y += ym * px1yz * p3d.y;
            outVec.z += zm * px1yz * p3d.z;

            p3d = GRADIENT(xp1y + zp1);
            outVec.x += xm1 * px1yz1 * p3d.x;
            outVec.y += ym * px1yz1 * p3d.y;
            outVec.z += zm1 * px1yz1 * p3d.z;

            p3d = GRADIENT(xp1y1 + zp);
            outVec.x += xm1 * px1y1z * p3d.x;
            outVec.y += ym1 * px1y1z * p3d.y;
            outVec.z += zm * px1y1z * p3d.z;

            p3d = GRADIENT(xp1y1 + zp1);
            outVec.x += xm1 * px1y1z1 * p3d.x;
            outVec.y += ym1 * px1y1z1 * p3d.y;
            outVec.z += zm1 * px1y1z1 * p3d.z;
